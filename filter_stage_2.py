...
def read_virus_taxids(virus_file_path):
    """Reads virus taxids from a file."""
    virus_taxids = set()
    try:
...
def process_line(line, virus_taxid_set):
    """Processes a single line and filters based on virus taxids."""
    if not line:
        return None
    try:
...
def extract_taxid_range_from_filename(filename):
    """Extracts taxid range from filename using regex."""
...
def process_files_serial(prot_file, virus_file_path, output_file_path):
    """Processes all files in a directory in serial, filtering protein IDs based on virus taxids."""
